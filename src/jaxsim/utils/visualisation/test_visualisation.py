...
# Load the iCub mode
model_path = "/home/carlotta/iit_ws/ergocub-software/urdf/ergoCub/robots/ergoCubSN001/model.urdf"
...
full_model = js.model.JaxSimModel.build_from_model_description(
    model_description=model_description, time_step=0.0001, is_urdf=True, contact_model = jaxsim.rbda.contacts.RelaxedRigidContacts.build()
)
...
# Creates coordinates 
coordinates = generate_equally_spaced_coordinates(n=number_of_robots, l=lengths_table_in_meters)
...
for time_istant in range(len(final_data_vector)):
    ...
    for idx_robot, name_i in enumerate(name_of_robots):
        ...
        idyntree_viz.update_model(np.array(data_i.joint_positions), np.array(data_i.base_transform),model_name=name_i, delta_x = coordinates_i[0], delta_y = coordinates_i[1])    
