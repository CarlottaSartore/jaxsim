...
class iDynTreeVisualizer:
    ...
    def modify_camera(self, length_board):
        ...
        center_point = iDynTree.Position(length_board / 2, length_board / 2, 0.0)  # Center of the chessboard (z = 0)
        camera_position =iDynTree.Position(length_board / 2, -length_board / 2 -3, 4.0)  # Position in front of the chessboard (x, y, z)
    ...
    def add_model(self, joint_name_list, base_link, urdf_path, model_name):
        mdlLoader = iDynTree.ModelLoader()
        mdlLoader.loadReducedModelFromFile(urdf_path, joint_name_list, base_link)
        self.viz.addModel(mdlLoader.model(), model_name)

    def add_multiple_instances(self,joint_name_list, base_link, urdf_path, model_name, number_of_robots, lengths_table_in_meters):
        self.coordinates = self.generate_equally_spaced_coordinates(n=number_of_robots, l=lengths_table_in_meters)
        self.name_of_robots = []
        for i in range(number_of_robots):
            name_i = "ergoCub"+str(i)
            self.name_of_robots.append(nam_i)
            idyntree_viz.add_model(joint_name_list=list(joints), base_link="root_link", urdf_path=model_path, model_name=name_i)
    ...
    def update_multiple_instances(data_i): 
        for idx_robot, name_i in enumerate(self.name_of_robots):
            ...
            idyntree_viz.update_model(np.array(data_robot_i.joint_positions), np.array(data_robot_i.base_transform),model_name=name_i, delta_x = coordinates_i[0], delta_y = coordinates_i[1])    
